...
drop function if exists get_actions_for;
---------------
-- DISTRIBUTION
---------------
...
DROP FUNCTION IF EXISTS get_actions;
CREATE FUNCTION get_actions_for(txid INT)
RETURNS TABLE (
	transaction_id bigint, 
	actions json
)
...
BEGIN RETURN QUERY 
    ...
    (
	SELECT 
		audit.logged_actions.client_query,
		audit.logged_actions.action,
		audit.logged_actions.transaction_id as tx_id,
		audit.logged_actions.row_data->'id' as id, 
		audit.logged_actions.row_data-> 'coord' as coord, 
		audit.logged_actions.row_data->'distribution' as dist,
		CASE WHEN audit.logged_actions.action = 'UPDATE' THEN
			audit.logged_actions.changed_fields -> 'distribution'
		ELSE 
			''
		END AS new_dist
	FROM 
	...
	WHERE 
		audit.logged_actions.table_name='versionpin' AND 
		audit.logged_actions.row_data IS NOt null AND 
		audit.logged_actions.transaction_id=tx_id AND 
